		...
		for (int i = args.length; i >= 0; i--) {
			...
			if (commandMap.containsKey(cmdLabel)) {
				...
				if (command.permission() != "" && !sender.hasPermission(command.permission())) {
					sender.sendMessage(ChatUtils.colorize(Configuration.language.noPermissionMessage));
					return true;
				}
				if (command.inGameOnly() && !(sender instanceof Player)) {
					sender.sendMessage(ChatUtils.colorize("This command is only performable in game."));
					return true;
				}
				if (command.inConsoleOnly() && !(sender instanceof ConsoleCommandSender)) {
					sender.sendMessage(ChatUtils.colorize("This command is only performable in Console."));
					return true;
				}
				try {
